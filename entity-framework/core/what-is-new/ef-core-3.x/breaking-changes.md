...
Le modifiche alle API e al comportamento seguenti possono causare l'interruzione delle applicazioni esistenti durante l'aggiornamento a 3. x.
...
| [La connessione al database viene ora chiusa se non viene più usata prima del completamento di TransactionScope](#dbc) | Basso      |
...
| [~~EF Core 3,0 destinazioni .NET Standard 2,1 anziché .NET Standard 2,0~~ Ripristinato](#netstandard21) | |
| [~~L'esecuzione di query viene registrata a livello di debug~~ - Modifica annullata](#qe) | |

## <a name="high-impact-changes"></a>Modifiche a elevato effetto

### <a name="linq-queries-are-no-longer-evaluated-on-the-client"></a>Le query LINQ non vengono più valutate nel client

[Rilevamento del problema #14935](https://github.com/dotnet/efcore/issues/14935) 
 [Vedere anche problema #12795](https://github.com/dotnet/efcore/issues/12795)
...
Prima di questa modifica, per ottenere EF Core erano necessarie procedure diverse, a seconda che l'applicazione avesse o meno come destinazione ASP.NET Core e SQL Server.
Inoltre, l'aggiornamento di ASP.NET Core forzava l'aggiornamento di EF Core e del provider di SQL Server, non sempre auspicabile.
...
[Problema n. 14016](https://github.com/dotnet/efcore/issues/14016)
...
[Rilevamento del problema #15392](https://github.com/dotnet/efcore/issues/15392)
...
[Problema n. 15704](https://github.com/dotnet/efcore/issues/15704)
...
[Problema n. 13518](https://github.com/dotnet/efcore/issues/13518)
...
[Problema n. 12378](https://github.com/dotnet/efcore/issues/12378)
...
[Problema n. 14616](https://github.com/dotnet/efcore/issues/14616)
...
[Problema n. 10114](https://github.com/dotnet/efcore/issues/10114)
...
Questa modifica è stata apportata per migliorare l'esperienza di associazione di dati e degli scenari di controllo in cui è importante individuare le entità che verranno eliminate _prima della chiamata a_ `SaveChanges`.
...
[Rilevamento del problema #18022](https://github.com/dotnet/efcore/issues/18022)

#### <a name="old-behavior"></a>Comportamento precedente

Prima del 3,0, il caricamento con entusiasmo degli spostamenti delle raccolte tramite `Include` gli operatori causava la generazione di più query nel database relazionale, una per ogni tipo di entità correlato.
...
Sebbene tecnicamente non si tratti di una modifica di rilievo, potrebbe avere un impatto significativo sulle prestazioni dell'applicazione quando una singola query contiene un numero elevato di operatori per le esplorazioni della `Include` raccolta. Per ulteriori informazioni e per riscrivere le query in modo più efficiente, [vedere il commento](https://github.com/dotnet/efcore/issues/18022#issuecomment-542397085) .
...
[Problema n. 12661](https://github.com/dotnet/efcore/issues/12661)
...
[Problema n. 14194](https://github.com/dotnet/efcore/issues/14194)
...
* **`ModelBuilder.Query<>()`** -È invece necessario `ModelBuilder.Entity<>().HasNoKey()` chiamare il metodo per contrassegnare un tipo di entità senza chiavi.
...
* **`DbQuery<>`** - `DbSet<>` È invece consigliabile usare.
* **`DbContext.Query<>()`** - `DbContext.Set<>()` È invece consigliabile usare.
* **`IQueryTypeConfiguration<TQuery>`** - `IEntityTypeConfiguration<TEntity>` È invece consigliabile usare.
...
[Rilevamento del problema #12444](https://github.com/dotnet/efcore/issues/12444) 
 [Rilevamento del problema #9148](https://github.com/dotnet/efcore/issues/9148) 
 [Rilevamento del problema #14153](https://github.com/dotnet/efcore/issues/14153)
...
[Problema n. 9005](https://github.com/dotnet/efcore/issues/9005)
...
[Problema n. 14154](https://github.com/dotnet/efcore/issues/14154)
...
[Rilevamento del problema #18876](https://github.com/dotnet/efcore/issues/18876)
...
In caso contrario `AsNoTracking()` , aggiungere una chiamata:
...
[Problema n. 13998](https://github.com/dotnet/efcore/issues/13998)
...
[Problema n. 13274](https://github.com/dotnet/efcore/issues/13274)
...
[Problema n. 14218](https://github.com/dotnet/efcore/issues/14218)
...
[Problema n. 6872](https://github.com/dotnet/efcore/issues/6872)
...
[Problema n. 12430](https://github.com/dotnet/efcore/issues/12430)
...
[Problema n. 12523](https://github.com/dotnet/efcore/issues/12523)
...
In una versione futura di EF Core dopo il 3,0, si prevede di abilitare di nuovo in modo esplicito il nome di un campo diverso dal nome della proprietà (vedere il problema [#15307](https://github.com/dotnet/efcore/issues/15307)):
...
[Problema n. 14756](https://github.com/dotnet/efcore/issues/14756)

#### <a name="old-behavior"></a>Comportamento precedente

Prima di EF Core 3,0, chiamando `AddDbContext` o `AddDbContextPool` sarebbero stati registrati anche i servizi di memorizzazione nella cache e di registrazione della memoria con le chiamate a [AddLogging](/dotnet/api/microsoft.extensions.dependencyinjection.loggingservicecollectionextensions.addlogging) e [AddMemoryCache](/dotnet/api/microsoft.extensions.dependencyinjection.memorycacheservicecollectionextensions.addmemorycache).
...
Se l'applicazione necessita di questi servizi, registrarli in modo esplicito con il contenitore di inserimento delle dipendenze usando [AddLogging](/dotnet/api/microsoft.extensions.dependencyinjection.loggingservicecollectionextensions.addlogging) o [AddMemoryCache](/dotnet/api/microsoft.extensions.dependencyinjection.memorycacheservicecollectionextensions.addmemorycache).
...
[Rilevamento del problema #12905](https://github.com/dotnet/efcore/issues/12905)

#### <a name="old-behavior"></a>Comportamento precedente

Prima di EF Core 3,0, i metodi di chiamata `AddEntityFramework*` registravano anche i servizi di memorizzazione nella cache di memoria con di senza un limite di dimensioni.

#### <a name="new-behavior"></a>Nuovo comportamento

A partire da EF Core 3,0, registrerà `AddEntityFramework*` un servizio IMemoryCache con un limite di dimensioni. Se altri servizi aggiunti successivamente dipendono da IMemoryCache, è possibile raggiungere rapidamente il limite predefinito che causa eccezioni o prestazioni ridotte.
...
Nella maggior parte dei casi, la chiamata a `AddEntityFramework*` non è necessaria se `AddDbContext` `AddDbContextPool` viene chiamato anche o. Pertanto, la migliore mitigazione consiste nel rimuovere la `AddEntityFramework*` chiamata.

Se l'applicazione necessita di questi servizi, registrare un'implementazione di IMemoryCache in modo esplicito con il contenitore DI inserimento in anticipo usando [AddMemoryCache](/dotnet/api/microsoft.extensions.dependencyinjection.memorycacheservicecollectionextensions.addmemorycache).

<a name="dbe"></a>

### <a name="dbcontextentry-now-performs-a-local-detectchanges"></a>DbContext.Entry esegue ora un DetectChanges locale

[Problema n. 13552](https://github.com/dotnet/efcore/issues/13552)
...
[Problema n. 14617](https://github.com/dotnet/efcore/issues/14617)
...
[Problema n. 14698](https://github.com/dotnet/efcore/issues/14698)
...
[Problema n. 12780](https://github.com/dotnet/efcore/issues/12780)
...
[Problema n. 10236](https://github.com/dotnet/efcore/issues/10236)
...
[Problema n. 9171](https://github.com/dotnet/efcore/issues/9171)
...
[Problema n. 15184](https://github.com/dotnet/efcore/issues/15184)
...
[Problema n. 9913](https://github.com/dotnet/efcore/issues/9913)
...
[Problema n. 11811](https://github.com/dotnet/efcore/issues/11811)
...
[Problema n. 12366](https://github.com/dotnet/efcore/issues/12366)
...
[Problema n. 214](https://github.com/dotnet/efcore/issues/214)
...
[Problema n. 12151](https://github.com/dotnet/efcore/issues/12151)
...
[Problema n. 15078](https://github.com/dotnet/efcore/issues/15078)
...
[Problema n. 15020](https://github.com/dotnet/efcore/issues/15020)
...
[Problema n. 12978](https://github.com/dotnet/efcore/issues/12978)
...
[Problema n. 16400](https://github.com/dotnet/efcore/issues/16400)
...
È consigliabile eseguire l'aggiornamento a una versione più recente di SQL Server o usare un livello di compatibilità superiore, in modo che il codice SQL generato sia supportato. Detto questo, se non è possibile procedere in questo modo, [aggiungere un commento per il problema](https://github.com/dotnet/efcore/issues/16400) con indicazioni dettagliate. Microsoft potrebbe rivedere questa decisione in base ai commenti e suggerimenti.
...
[Problema n. 16119](https://github.com/dotnet/efcore/issues/16119)
...
[Problema n. 10985](https://github.com/dotnet/efcore/issues/10985)
...
[Problema n. 10730](https://github.com/dotnet/efcore/issues/10730)
...
[Problema n. 15997](https://github.com/dotnet/efcore/issues/15997)
...
[Problema n. 11506](https://github.com/dotnet/efcore/issues/11506)
...
[Problema n. 14824](https://github.com/dotnet/efcore/issues/14824)
...
[Problema n. 14825](https://github.com/dotnet/efcore/issues/14825)
...
[Rilevamento del problema #15636](https://github.com/dotnet/efcore/issues/15636)
...
[Problema n. 13573](https://github.com/dotnet/efcore/issues/13573)
...
[Rilevamento del problema #12757](https://github.com/dotnet/efcore/issues/12757)

#### <a name="old-behavior"></a>Comportamento precedente

Un DbFunction configurato con schema come stringa vuota è stato trattato come funzione predefinita senza uno schema. Il codice seguente, ad esempio, eseguirà il mapping della `DatePart` funzione CLR alla `DATEPART` funzione predefinita in SqlServer.
...
### <a name="ef-core-30-targets-net-standard-21-rather-than-net-standard-20-reverted"></a>~~EF Core 3,0 destinazioni .NET Standard 2,1 anziché .NET Standard 2,0~~ Ripristinato

[Problema n. 15498](https://github.com/dotnet/efcore/issues/15498)

EF Core 3,0 destinazioni .NET Standard 2,1, che è una modifica di rilievo che esclude .NET Framework applicazioni. EF Core 3,1 è stato ripristinato e le destinazioni .NET Standard 2,0.
...
[Problema n. 14523](https://github.com/dotnet/efcore/issues/14523)
