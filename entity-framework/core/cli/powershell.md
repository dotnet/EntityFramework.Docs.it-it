...
Se non si usa Visual Studio, è consigliabile usare gli [strumenti da riga di comando EF Core](xref:core/cli/dotnet) . Gli strumenti interfaccia della riga di comando di .NET Core sono multipiattaforma ed eseguiti all'interno di un prompt dei comandi.
...
Per specificare [l'ambiente per i](/aspnet/core/fundamentals/environments) progetti ASP.NET Core, impostare **env: ASPNETCORE_ENVIRONMENT** prima di eseguire i comandi.
...
Nell'esempio seguente viene creato uno script per la migrazione di InitialCreate (da un database senza migrazioni), usando il nome della migrazione.
