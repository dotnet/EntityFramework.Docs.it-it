...
> [!TIP]
> <span data-ttu-id="b418d-104">Il codice in questo documento è disponibile in GitHub come [esempio eseguibile](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/).</span><span class="sxs-lookup"><span data-stu-id="b418d-104">The code in this document can be found on GitHub as a [runnable sample](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/).</span></span>
...
<span data-ttu-id="b418d-109">L' [esempio](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/) contiene due progetti:</span><span class="sxs-lookup"><span data-stu-id="b418d-109">The [sample](https://github.com/dotnet/EntityFramework.Docs/tree/master/samples/core/Miscellaneous/Testing/ItemsWebApi/) contains two projects:</span></span>

- <span data-ttu-id="b418d-110">ItemsWebApi: un' [API Web molto semplice supportata da ASP.NET Core](/aspnet/core/tutorials/first-web-api) con un singolo controller</span><span class="sxs-lookup"><span data-stu-id="b418d-110">ItemsWebApi: A very simple [Web API backed by ASP.NET Core](/aspnet/core/tutorials/first-web-api) with a single controller</span></span>
...
<span data-ttu-id="b418d-113">Il modello che supporta questa API è costituito da due tipi di entità: Items e Tags .</span><span class="sxs-lookup"><span data-stu-id="b418d-113">The model backing this API has two entity types: Items and Tags.</span></span>

- <span data-ttu-id="b418d-114">Items hanno un nome con distinzione tra maiuscole e minuscole e una raccolta di Tags .</span><span class="sxs-lookup"><span data-stu-id="b418d-114">Items have a case-sensitive name and a collection of Tags.</span></span>
- <span data-ttu-id="b418d-115">Ogni Tag ha un'etichetta e un conteggio che rappresenta il numero di volte in cui è stato applicato a Item .</span><span class="sxs-lookup"><span data-stu-id="b418d-115">Each Tag has a label and a count representing the number of times it has been applied to the Item.</span></span>
- <span data-ttu-id="b418d-116">Ogni Item deve avere un solo Tag oggetto con un'etichetta specificata.</span><span class="sxs-lookup"><span data-stu-id="b418d-116">Each Item should only have one Tag with a given label.</span></span>
...
- <span data-ttu-id="b418d-118">L'eliminazione di un oggetto Item deve eliminare tutti gli associati Tags .</span><span class="sxs-lookup"><span data-stu-id="b418d-118">Deleting an Item should delete all associated Tags.</span></span>

#### <a name="the-no-locitem-entity-type"></a><span data-ttu-id="b418d-119">ItemTipo di entità</span><span class="sxs-lookup"><span data-stu-id="b418d-119">The Item entity type</span></span>

<span data-ttu-id="b418d-120">`Item`Tipo di entità:</span><span class="sxs-lookup"><span data-stu-id="b418d-120">The `Item` entity type:</span></span>
...
- <span data-ttu-id="b418d-127">Tags vengono esposti come `IReadOnlyList<Tag>` per impedire la modifica arbitraria.</span><span class="sxs-lookup"><span data-stu-id="b418d-127">Tags are exposed as a `IReadOnlyList<Tag>` to prevent arbitrary modification.</span></span>
  - <span data-ttu-id="b418d-128">EF associa la `Tags` proprietà al `_tags` campo sottostante mediante la corrispondenza dei nomi.</span><span class="sxs-lookup"><span data-stu-id="b418d-128">EF associates the `Tags` property with the `_tags` backing field by matching their names.</span></span>
  - <span data-ttu-id="b418d-129">Il `AddTag` metodo accetta un'etichetta tag e implementa la regola business descritta in precedenza.</span><span class="sxs-lookup"><span data-stu-id="b418d-129">The `AddTag` method takes a tag label and implements the business rule described above.</span></span>
...
- <span data-ttu-id="b418d-132">La `Tags` proprietà di navigazione è configurata per una relazione molti-a-uno</span><span class="sxs-lookup"><span data-stu-id="b418d-132">The `Tags` navigation property is configured for a many-to-one relationship</span></span>
  - <span data-ttu-id="b418d-133">Non è necessario disporre di una proprietà di navigazione da Tag a Item , pertanto non è inclusa.</span><span class="sxs-lookup"><span data-stu-id="b418d-133">There is no need for a navigation property from Tag to Item, so it is not included.</span></span>
  - <span data-ttu-id="b418d-134">Inoltre, non Tag definisce una proprietà di chiave esterna.</span><span class="sxs-lookup"><span data-stu-id="b418d-134">Also, Tag does not define a foreign key property.</span></span>
...
#### <a name="the-no-loctag-entity-type"></a><span data-ttu-id="b418d-136">TagTipo di entità</span><span class="sxs-lookup"><span data-stu-id="b418d-136">The Tag entity type</span></span>

<span data-ttu-id="b418d-137">`Tag`Tipo di entità:</span><span class="sxs-lookup"><span data-stu-id="b418d-137">The `Tag` entity type:</span></span>
...
<span data-ttu-id="b418d-139">In modo analogo a Item , Tag nasconde la chiave primaria e rende la proprietà di sola `Label` lettura.</span><span class="sxs-lookup"><span data-stu-id="b418d-139">Similarly to Item, Tag hides its primary key and makes the `Label` property read-only.</span></span>

### <a name="the-no-locitemscontroller"></a><span data-ttu-id="b418d-140">Il Items controller</span><span class="sxs-lookup"><span data-stu-id="b418d-140">The ItemsController</span></span>
...
<span data-ttu-id="b418d-143">Dispone di metodi per ottenere tutti Items o un oggetto Item con un nome specificato:</span><span class="sxs-lookup"><span data-stu-id="b418d-143">It has methods to get all Items or an Item with a given name:</span></span>

[!code-csharp[Get](../../../samples/core/Miscellaneous/Testing/ItemsWebApi/ItemsWebApi/Controllers/ItemsController.cs?name=Get)]

<span data-ttu-id="b418d-144">Dispone di un metodo per aggiungere un nuovo Item :</span><span class="sxs-lookup"><span data-stu-id="b418d-144">It has a method to add a new Item:</span></span>
...
<span data-ttu-id="b418d-145">Metodo per contrassegnare un oggetto Item con un'etichetta:</span><span class="sxs-lookup"><span data-stu-id="b418d-145">A method to tag an Item with a label:</span></span>
...
<span data-ttu-id="b418d-146">E un metodo per eliminare un oggetto Item e tutti gli associati Tags :</span><span class="sxs-lookup"><span data-stu-id="b418d-146">And a method to delete an Item and all associated Tags:</span></span>
...
- <span data-ttu-id="b418d-158">`Can_remove_item_and_all_associated_tags` Quando si esegue con il provider di database in memoria EF</span><span class="sxs-lookup"><span data-stu-id="b418d-158">`Can_remove_item_and_all_associated_tags` when running with the EF in-memory database provider</span></span>
...
<span data-ttu-id="b418d-160">Questo aspetto viene trattato in modo più dettagliato di seguito.</span><span class="sxs-lookup"><span data-stu-id="b418d-160">This is covered in more detail below.</span></span>
...
<span data-ttu-id="b418d-188">Due test leggermente più necessari coprono la logica di business per l'aggiunta di tags .</span><span class="sxs-lookup"><span data-stu-id="b418d-188">Two slightly more involved tests cover the business logic around adding tags.</span></span>
...
<span data-ttu-id="b418d-194">Uno dei requisiti per l'applicazione è che " Items hanno un nome con distinzione tra maiuscole e minuscole e una raccolta di Tags ".</span><span class="sxs-lookup"><span data-stu-id="b418d-194">One of the requirements for our application is that "Items have a case-sensitive name and a collection of Tags."</span></span>
...
<span data-ttu-id="b418d-205">Un altro requisito per l'applicazione è che "l'eliminazione di un Item dovrebbe eliminare tutti gli associati Tags ".</span><span class="sxs-lookup"><span data-stu-id="b418d-205">Another of the requirements for our application is that "deleting an Item should delete all associated Tags."</span></span>
